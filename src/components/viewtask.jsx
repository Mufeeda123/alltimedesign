  ...
  const handleDeleteTask = async (taskId) => {
    ...
    try {
      const response = await axios.delete(
        `https://stage.api.sloovi.com/task/lead_65b171d46f3945549e3baa997e3fc4c2/${taskId}?company_id=company_0f8d040401d14916bc2430480d7aa0f8`,
        {
          headers: {
            'Authorization': `Bearer ${tokenValue}`,
            'Accept': 'application/json',
              ...
              {tasks.map((task) => (
                <div key={task.id} className="bg-gray-200 text-gray-800 border border-gray-400 p-4 mb-4">
                  ...
                  <div className="flex justify-end mt-2">
                    <button className="bg-blue-500 text-white px-4 py-2 rounded mr-2" onClick={() => editfunction(task.id)}>Edit</button>
                    <button className="bg-red-500 text-white px-4 py-2 rounded" onClick={() => deleteTask(task.id)}>Delete</button>
                  </div>
                </div>
              ))}
            </ul>
